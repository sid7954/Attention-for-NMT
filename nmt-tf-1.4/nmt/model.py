...
#import nmt.joint_attention_new as joint_attn
import nmt.joint_attention_new1 as joint_attn
...
class BaseModel(object):
  ...
  def build_graph(self, hparams, scope=None):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
	  ...
	  #Override loss
          print ("OL fn is ", self._override_loss_fn, logits)
     	  if (self._override_loss_fn is not None) or False:
            #loss = self._override_loss_fn(logits, final_context_state, self.mode != tf.contrib.learn.ModeKeys.INFER)
	    loss =  -tf.reduce_sum(tf.reduce_logsumexp(final_context_state.alphas, axis=1))

            if self.mode == tf.contrib.learn.ModeKeys.EVAL:
              loss = tf.Print(loss, [loss], "LossE is ")
            else:
              loss = tf.Print(loss, [loss], "LossT is ")
...
class Model2(BaseModel):
  ...
  def _override_loss_fn(self, logits, final_context_state, not_infer) :
          return tf.reduce_sum(tf.reduce_logsumexp(final_context_state.alphas, axis=1))
  ...
  def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
    ...
    print ("Joint is invoed");
    cell = joint_attn.JointAttentionDecoder(
                            cell,
                            ...
                            is_training = not(self.mode == tf.contrib.learn.ModeKeys.INFER),
                            name="jointDecoder")
    ...
    decoder_initial_state = cell.zero_state(batch_size, dtype).clone(
          cell_state= cell._unstack_beams(tf.contrib.seq2seq.tile_batch(encoder_state, cell.k)))
